...
acquire 阶段报 → 立即重建、本帧跳过；present 阶段报 → 图已交出，标记待重建；SUBOPTIMAL → 画完这帧、标 pending。**2026-09-22 起重建时机由 `host.rs` 的 `ResizeGate` 统一节拍**：最小化整帧让路（对 stale swapchain 的 acquire 在 FIFO 下无限阻塞——最小化后无法还原的死锁根因）；尺寸变化只标 pending、帧首按需重建，配 MAILBOX present mode（frender 同款）让拖拽中帧循环持续流动。机制与实测见《[窗口闸门：最小化死锁与resize卡顿的实测与修复](窗口闸门：最小化死锁与resize卡顿的实测与修复.md)》。`rebuild` 里的 `device_wait_idle` 是 CPU 唯一一次全量等 GPU（在飞帧还引用旧 image，拆早了是未定义行为）。
