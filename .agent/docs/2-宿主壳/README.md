...
- 《[窗口闸门：最小化死锁与resize卡顿的实测与修复.md](材料/窗口闸门：最小化死锁与resize卡顿的实测与修复.md)》——窗口缺陷专项（2026-09-22，3.1 收官后）：两个用户可见 bug 的根因与修复——**最小化后无法还原 = stale swapchain acquire 无限阻塞冻结消息泵（自锁死锁）**、**拖拽卡顿 = 每步 ~60ms 的 wait_idle+destroy+create 同步重建**；两轮返工（整帧让路版被用户否决）后定案 frenderer 三件套移植：MAILBOX present mode + pending 帧首按需重建 + `WinitSettings::Reactive(1/60)` 节流，外加 bevy_winit fork 内修（`Resized(0,0)` 不写 Window 组件）；附脚本化窗口实测的四颗钉子（多窗口冒充/CharSet/ShowWindowAsync 绕行/PS 闭包）
