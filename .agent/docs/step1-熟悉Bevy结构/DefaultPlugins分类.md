...
3. 唯一的琥珀项 `PbrPlugin` 已核实**优雅降级**：build 里全是 `if let Some(render_app) = app.get_sub_app_mut(RenderApp)` 模式（`bevy_pbr/src/lib.rs:301/332/357`），缺 RenderApp 时渲染部分静默跳过，而 `Assets<StandardMaterial>` 的注册发生在主世界——**保留它**，正好是取数表需要的材质容器。
